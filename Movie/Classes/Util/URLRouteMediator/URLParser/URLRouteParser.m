...
/// 路由URL中跳转类型
@property (assign,nonatomic) URLRouteActionType  actionType;
/// 是否标准的路由
@property (assign,nonatomic) BOOL  isRouteURL;
@end


@implementation URLRouteParser

+ (instancetype)urlRoutePaeserWithURL:(NSString *)url {
    URLRouteParser *urlRoutePaeser = [[self alloc] init];
...
- (void)parseURL {
    ...
    // 校验scheme
    if ([URLRouteAction.sharedInstance.scheme isNotBlank]) {
        self.isRouteURL = [url.scheme isEqualToString:URLRouteAction.sharedInstance.scheme];
    }else{
        self.isRouteURL = [url.scheme isEqualToString:@"http"] || [url.scheme isEqualToString:@"https"];
    }
    // 校验host
    if ([URLRouteAction.sharedInstance.host isNotBlank]) {
        self.isRouteURL = [url.host isEqualToString:URLRouteAction.sharedInstance.host];
    }
    // 解析URL
    ...
    if ([URLRouteAction.sharedInstance.signSalt isNotBlank]) { // 开启签名校验
        // 生成的签名串
        NSString *signatureStr = [URLRouteAction mapSignCheck:paramInfo];
        // 将生成的签名串与路由中的签名串比对
